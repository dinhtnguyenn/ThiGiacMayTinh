...
- A profile can contain up to five enrollment samples by default. Select an existing profile on the demo registration screen to add a varied capture, such as straight ahead, slight left/right turn, and normal lighting. The server uses the selected stable profile ID rather than the editable name, then compares a recognition embedding with every stored sample and selects the highest similarity. Outside this classroom demo, do not expose that selector publicly: require an administrator or a per-profile enrollment code to prevent visitors from contaminating another person's profile.
