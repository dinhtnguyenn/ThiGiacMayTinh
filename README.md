...
| Feature | Behavior |
| --- | --- |
| Face registration | The user captures from camera or chooses an image, reviews a cropped face preview, then explicitly confirms before a quality-checked InsightFace embedding is written to the server. For this course demo, the public registration screen can also select an existing profile to add another capture. |
...
3. **Quản lý dữ liệu** — enter the server administrator token to view the directory, inspect each profile's stored crop images and sample metadata, rename a profile, or delete one. The token is kept only in the tab's memory and is never written to browser storage.

The UI sends camera frames or browser-selected image files only to process the immediate action. While a camera is live, supported browsers use their local Face Detector API for a fast box overlay while InsightFace on the server determines the final recognition result. Recognition frames are resized to a 512 px maximum; the less frequent registration capture uses 640 px for better quality checks. Camera frames and source uploads are never stored. After `Xác nhận`, the server uses InsightFace's five landmarks with `face_align.norm_crop()` to rotate the face upright and stores one normalized 112x112 JPEG crop alongside its embedding; this aligned crop is available only to an authenticated administrator in the detail view. The registration preview's embedding and crop exist only in the server process until confirmation or expiry; neither is written to SQLite before `Xác nhận`. After each registration or recognition, the processing trace is available on demand. The trace never includes raw images, embeddings, filesystem paths, or administrator tokens.

### Better enrollment and matching

- A profile can contain up to five enrollment samples by default. When selecting an existing profile on the demo registration screen, the submitted face must also match at least one stored sample of that profile before the preview is accepted and again before confirmation writes to SQLite. The default `FACEOPS_SELECTED_PROFILE_MATCH_THRESHOLD=0.55` is deliberately stricter than the normal recognition threshold. Outside this classroom demo, do not expose that selector publicly: require an administrator or a per-profile enrollment code to prevent visitors from contaminating another person's profile.
- The maximum can be configured with `FACEOPS_MAX_SAMPLES_PER_PROFILE`. Administrators can inspect and delete one poor sample. Deleting the final sample deletes the entire profile as well, so its original embedding cannot remain searchable without a visible sample. Do not turn the limit into an unbounded value: a small, curated set of high-quality and varied samples is more useful than many nearly identical frames.
...
- The browser does not persist a face embedding, image, profile, workspace token, administrator token, or enrollment continuation token. During the active registration tab only, it holds the continuation token in memory to let the person add their own varied samples; closing or reloading the tab clears it. The SQLite database in the server's persistent `/data` volume stores the name, InsightFace embedding, and one confirmed JPEG face crop for each new sample.
- Public recognition requests do not return a list of registered names. A name is returned only after a submitted face reaches the configured match threshold; the administrator-only management API and the explicitly marked course-demo selector are the exceptions.
- The course-demo registration screen intentionally exposes a minimal directory of names and sample counts so a student can select an existing profile and add a sample. This is a public demo exception, not an appropriate production authorization model; it never returns embeddings or stored face crops.
...
- Submitted images are decoded in memory to make an embedding. Only the confirmed face crop is written to SQLite; source camera frames and uploaded originals are not. Configure the reverse proxy and application logs so they do not capture request bodies, tokens, raw images, or stored crop data.
...
| `GET /api/registration-profiles` | Course-demo-only public selector containing profile IDs, names, and sample counts. It returns neither embeddings nor stored face crops. |
| `POST /api/registrations/{pending_id}/confirm` | Consumes the one-time preview and writes its embedding plus confirmed face crop to the shared server directory. An optional adjusted crop image is rechecked and becomes the saved sample. |
